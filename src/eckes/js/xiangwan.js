...
async function sign() {
    try {
        ...
        const encryptData = await common.sendRequest(`${baseUrl}/discover/sign`, 'post', headers, {
            "params": encryptCredentials(JSON.stringify({
                ...getCommonParams(),
                "days": days
            }))
...
async function getUserInfo() {
    try {
        const encryptData = await common.sendRequest(`${baseUrl}/me/member_info`, 'post', headers, {
            "params": encryptCredentials(JSON.stringify({
                ...getCommonParams()
            }))
        });
...
async function validateToken(token) {
    try {
        const encryptData = await common.sendRequest(`${baseUrl}/me/member_account`, 'post', {
