...
async function main() {
    ...
    await common.wait(common.getRandomWait(1200, 1800));
...
async function getUserInfo() {
    try {
        const data = await common.sendRequest(`${baseUrl}/server/api/authorized-login`, 'post', headers, {
            "union_id": $.unionId,
...
async function getDayList() {
    try {
        const data = await common.sendRequest(`${baseUrl}/server/api/day-list`, 'post', headers);
        if (200 !== data.code) {
            console.error(data.message);
            return;
        }
        ...
        console.log(`开始签到`);
        await common.wait(common.getRandomWait(1000, 1500));
        await signIn();
    } catch (e) {
...
async function signIn() {
    try {
        const data = await common.sendRequest(`${baseUrl}/server/api/day-sign`, 'post', headers);
        if (200 !== data.code) {
            console.error(data.message);
            return;
        }
...
async function receiveTaskWater(tid) {
    try {
        const data = await common.sendRequest(`${baseUrl}/server/api/task/receive-task-water`, 'post', headers, {
            "tid": tid
        });
        if (200 !== data.code) {
            console.error(data.message);
        ...
        if (3 === tid) {
            await common.wait(common.getRandomWait(15000, 15100));
        }
...
async function answerTaskWater(tid = 0) {
    try {
        await common.wait(common.getRandomWait(1000, 1500));
        const data = await common.sendRequest(`${baseUrl}/server/api/task/answer-task-water`, 'post', headers, {
            "tid": tid
        });
        await common.wait(common.getRandomWait(1000, 1500));
        if (200 !== data.code) {
...
async function joinPower() {
    try {
        const initialData = await common.sendRequest(`${baseUrl}/server/api/game/use-power`, 'post', headers, {
            "num": 1,
            "user_tree_id": $.treeId
        });
        ...
        while (remainingJoins > 0) {
            await common.wait(common.getRandomWait(1200, 1800));
            const responseData = await common.sendRequest(`${baseUrl}/server/api/game/use-power`, 'post', headers, {
                "num": 1,
                "user_tree_id": $.treeId
            });
...
async function receiveBox() {
    try {
        const data = await common.sendRequest(`${baseUrl}/server/api/game/receive-box`, 'post', headers);
        if (200 !== data.code) {
...
async function chooseInvest() {
    try {
        ...
        const data = await common.sendRequest(`${baseUrl}/server/api/power/choose-invest`, 'post', headers, {
            "condition": "min"
        });
        if (200 !== data.code) {
...
async function receiveInvest() {
    try {
        const data = await common.sendRequest(`${baseUrl}/server/api/power/receive-invest`, 'post', headers);
        if (200 !== data.code) {
